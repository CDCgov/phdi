...
def _validate_xml_elements(xml_elements, config_field) -> list:
    ...
    for xml_element in xml_elements:
        if _validate_xml_relatives(xml_element, config_field):
            continue
    ...
    for relative_config in relatives:
        if (
            _validate_xml_related_element(
                ...
                cda_path=config_field.get("cdaPath"),
...
def _validate_xml_related_element(xml_element, cda_path, relative_config) -> str:
    ...
    related_xml_elements = _get_xml_relative_iterator(
        cda_path,
        relative_cda_path,
        xml_element
    )
    ...
    if len(related_xml_elements) == 0:
        return None

    # first get the tag names and add them to the elements list
    for related_xml_element in related_xml_elements:
...
def _get_xml_relative_iterator(cda_path, relative_cda_path, xml_element):
    ...
    :param xml_element: The key xml element.
    :return: A list of relative xml elements to the key xml element.
    """

    # get the difference of the number of '/' between the key cda_path and the
    # relative_cda_path
    diff = len(cda_path.split(XML_PATH_DELIMITER))
...
def _validate_xml_attributes(xml_element, config_field) -> list:
    ...
    :return: A list of errors or and empty list.  If the list is empty then
...
def _get_xml_element_details(xml_element, config_field) -> str:
    ...
    as the details of the relative xml elements and formats this
    information into a single string for use in the validation
    messages.
    ...
    if relatives:
        ...
        for rel_config in relatives:
            ...
            # TODO:  not sure I like this pattern where the
            # actual xml element needs to be present to get
            # all the attribute details....it seems we may be
            # validating the relative xml elements multiple
            # times....
            rel_element = _validate_xml_related_element(
                xml_element,
                config_field.get("cdaPath"),
                rel_config
            )
            relative_attributes = (
                    ["Attributes:"]
                    + _get_xml_attributes(config_related_attributes, rel_element)
                    if config_related_attributes and rel_element
                    else []
                )
...
def _get_xml_attributes(xml_element, config_attributes) -> list:
    ...
    for attribute in config_attributes:
        ...
        reg_ex_string = f" with the required value patern: '{reg_ex}'" if reg_ex else ""
        ...
        attrs.append(f"'{attribute_name}'{reg_ex_string}{attribute_value}")
    return [", ".join(attrs)]
...
def _validate_xml_value(xml_element, config_field) -> list:
    ...
    if config_regex is not None:
        ...
        if pattern.match(value) is None:
            message = _get_ecr_custom_message(
                config_field,
                "The field value does exist or match the following pattern: "
                + config_regex
                + f". {_get_xml_element_details(xml_element, config_field)}",
            )
            return [message]
