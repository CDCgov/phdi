...
def standardize_hl7_datetimes(message: str) -> str:
    ...
    * Convert segment terminators back from ``\\r`` to ``\\n``
    ...
    :return: The HL7 message with potential problem formats resolved,
      if it was possible. If not, the original message.
    """
    ...
    try:
        ...
        _normalize_hl7_datetime_segment(parsed_message, "ORC", [9, 15, 27])

        # OBR-7 Observation date/time
        # OBR-8 Observation end date/time
        # OBR-22 Status change date/time
        # OBR-36 Scheduled date/time
        _normalize_hl7_datetime_segment(parsed_message, "OBR", [7, 8, 22, 36])

        # OBX-12 Effective date/time of reference range
        # OBX-14 Date/time of observation
        # OBX-19 Date/time of analysis
        _normalize_hl7_datetime_segment(parsed_message, "OBX", [12, 14, 19])

        # TQ1-7 Start date/time
        # TQ1-8 End date/time
        _normalize_hl7_datetime_segment(parsed_message, "TQ1", [7, 8])

        # SPM-18 Specimen received date/time
        # SPM-19 Specimen expiration date/time
        _normalize_hl7_datetime_segment(parsed_message, "SPM", [18, 19])
...
def convert_hl7_batch_messages_to_list(
    ...
    """
    Convert a batch file of messages into a list of strings
...
def _normalize_hl7_datetime_segment(
...
def _normalize_hl7_datetime(hl7_datetime: str) -> str:
    """
...
def _clean_hl7_batch(batch: str, delimiter: str = "\n") -> str:
    """
    Clean a batch file by replacing Windows (CR-LF) newlines with
    the specified newline delimiter (LF by default). Also, strip
