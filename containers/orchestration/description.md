...
The Orchestration can be run using Docker (or any other OCI container runtime e.g., Podman), or directly from the Python source code.

#### Running with Docker (Recommended)

To run the Orchestration with Docker, follow these steps.
...
Congratulations, the Orchestration should now be running on `localhost:8080`!

#### Running from Python Source Code

We recommend running the Orchestration from a container, but if that is not feasible for a given use-case, it may also be run directly from Python using the steps below.
...
5. Install all of the Python dependencies for the Orchestration with `pip install -r requirements.txt` into your virtual environment.
6. Run the Orchestration app on `localhost:8080` with `python -m uvicorn app.main:app --host 0.0.0.0 --port 8080`. 

### Building the Docker Image

To build the Docker image for the Orchestration from source instead of downloading it from the PHDI repository follow these steps.
...
When viewing these docs from the `/redoc` endpoint on a running instance of the Orchestration or the PHDI website, detailed documentation on the API will be available below. 
