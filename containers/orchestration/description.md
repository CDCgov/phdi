...
### Running the /process-message endpoint
When processing an ecr .zip file through the Orchestration app, call the `/process-message` endpoint to run the file. 

The endpoint will be a POST call to the `/process-message` endpoint with the following parameters in the form body
...
```
curl --location 'https://your_url_here/orchestration/process-message' \
