...
@mock.patch("app.cloud.azure.DefaultAzureCredential")
...
@mock.patch("app.cloud.azure.SecretClient")
@mock.patch("app.cloud.azure.DefaultAzureCredential")
...
@mock.patch("app.cloud.gcp.google.auth.transport.requests.Request")
@mock.patch("app.cloud.gcp.google.auth.default")
...
def test_azure_upload_object(mock_get_client):
    ...
    app_container_client = AzureCloudContainerConnection(
    ...
    app_container_client.upload_object(
        object_content_json,
        object_container,
        object_path,
    )
...
def test_azure_download_object(mock_get_client):
    ...
    download_content = app_container_client.download_object(
        object_container,
        object_path,
    )
...
def test_azure_download_object_cp1252(mock_get_client):
    ...
    with open(
        pathlib.Path(__file__).parent / "assets" / "cp1252-sample.txt",
        "rb",
    ) as cp1252file:
...
@mock.patch("app.cloud.azure.BlobServiceClient")
def test_azure_list_containers(mock_service_client):
    ...
    container_list = app_container_client.list_containers()
...
def test_azure_list_objects(mock_get_client):
    ...
    blob_list = app_container_client.list_objects(object_container, object_prefix)
...
def test_azure_blob_exists(mock_get_client):
    ...
    exists = app_container_client.blob_exists(object_container, filename)
...
def test_gcp_storage_connect_init():
    app_container_client = GcpCloudStorageConnection()
    assert app_container_client._GcpCloudStorageConnection__storage_client is None


@mock.patch("app.cloud.gcp.storage")
def test_gcp_get_storage_client(patched_storage):
    app_container_client = GcpCloudStorageConnection()
    app_container_client._get_storage_client()
...
def test_gcp_list_objects(mock_get_client):
    ...
    object_bucket = "some-container"
    app_storage_client = GcpCloudStorageConnection()
    blob_list = app_storage_client.list_objects(object_bucket)
...
def test_gcp_list_containers(mock_get_client):
    ...
    app_storage_client = GcpCloudStorageConnection()
    bucket_list = app_storage_client.list_containers()
