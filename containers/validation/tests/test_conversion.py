...
import pytest
from app.fhir.conversion import convert_to_fhir
from app.fhir.conversion.convert import _get_fhir_conversion_settings
from app.fhir.conversion.convert import add_rr_data_to_eicr
from app.fhir.conversion.convert import ConversionError
from app.harmonization import standardize_hl7_datetimes
...
def test_get_fhir_conversion_settings():
    ...
    message = ""
    with open(pathlib.Path(__file__).parent / "assets" / "sample_hl7.hl7") as fp:
        message = fp.read()
    ...
    message = ""
    with open(
        pathlib.Path(__file__).parent / "assets" / "hl7_with_msh_3_set.hl7"
    ) as fp:
        message = fp.read()
    settings = _get_fhir_conversion_settings(message)
    assert settings == {
    ...
    message = ""
    with open(pathlib.Path(__file__).parent / "assets" / "ccda_sample.xml") as fp:
        message = fp.read()
    settings = _get_fhir_conversion_settings(message)
    assert settings == {
    ...
    message = ""
    with open(
        pathlib.Path(__file__).parent / "assets" / "ccda_sample_unknowntype.xml"
    ) as fp:
        message = fp.read()
